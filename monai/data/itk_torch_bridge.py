...
def itk_to_monai_affine(image, matrix, translation, center_of_rotation=None, reference_image=None) -> torch.Tensor:
    """
    Converts an ITK affine matrix (2x2 for 2D or 3x3 for 3D matrix and translation vector) to a MONAI_ affine matrix.
    ...
    # Adjust based on spacing. It is required because MONAI_ does not update the
...
def monai_to_itk_affine(image, affine_matrix, center_of_rotation=None):
    """
    Converts a MONAI_ affine matrix to an ITK affine matrix (2x2 for 2D or 3x3 for
    3D matrix and translation vector). See also 'itk_to_monai_affine'.

    Args:
        image: The ITK image object. This is used to extract the spacing and direction information.
        affine_matrix: The 3x3 for 2D or 4x4 for 3D MONAI_ affine matrix.
        center_of_rotation: The center of rotation. If provided, the affine
                            matrix will be adjusted to account for the difference
                            between the center of the image and the center of rotation.
...
def get_itk_image_center(image):
    ...
    This center is equivalent to the implicit image center that MONAI_ uses.
...
def _assert_itk_regions_match_array(image):
    ...
    if not indices_are_zeros:
        raise AssertionError("ITK-MONAI_ bridge: non-zero ITK region indices encountered")
    if not sizes_match:
        raise AssertionError("ITK-MONAI_ bridge: ITK regions should be of the same shape")
...
def _compute_reference_space_affine_matrix(image, ref_image) -> torch.Tensor:
    ...
    # Convert matrix ITK matrix and translation to MONAI_ affine matrix
...
def monai_to_itk_ddf(image, ddf):
    """
    converting the dense displacement field from the MONAI_ space to the ITK
    Args:
