...
class PersistentDataset(Dataset):
    ...
    def set_transform_hash(self, hash_xform_func: Callable[..., bytes]):
        ...
        inherit from MONAI_'s `Transform` class."""
        hashable_transforms = []
...
class SmartCacheDataset(Randomizable, CacheDataset):
    ...
    Note:
        ...
        If using MONAI_ workflows, please add `SmartCacheHandler` to the handler list of trainer,
...
class ZipDataset(Dataset):
    ...
    def _transform(self, index: int):
        ...
        # use tuple instead of list as the default collate_fn callback of MONAI_ DataLoader flattens nested lists
        return tuple(data)
