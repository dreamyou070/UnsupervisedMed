...
def list_data_collate(batch: Sequence):
    ...
    try:
        ...
        if "equal size" in re_str:
            ...
            re_str += (
                "\n\nMONAI_ hint: if your transforms intentionally create images of different shapes, creating your "
        ...
        if "numpy" in re_str and "Tensor" in re_str:
            if key is not None:
                re_str += f"\nCollate error on the key '{key}' of dictionary data."
            re_str += (
                "\n\nMONAI_ hint: if your transforms intentionally create mixtures of torch Tensor and numpy ndarray, "
...
def pad_list_data_collate(batch: Sequence, method: str = Method.SYMMETRIC, mode: str = NumpyPadMode.CONSTANT, **kwargs):
    ...
    Same as MONAI_'s ``list_data_collate``, except any tensors are centrally padded to match the shape of the biggest
