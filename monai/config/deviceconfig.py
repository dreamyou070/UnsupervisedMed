...
def get_config_values():
    ...
    output["MONAI_"] = monai.__version__
...
def print_config(file=sys.stdout):
    ...
    print(f"MONAI_ flags: HAS_EXT = {HAS_EXT}, USE_COMPILED = {USE_COMPILED}, USE_META_DICT = {USE_META_DICT}")
    print(f"MONAI_ rev id: {monai.__revision_id__}")
    username = getpass.getuser()
    masked_file_path = re.sub(username, "<username>", monai.__file__)
    print(f"MONAI_ __file__: {masked_file_path}", file=file, flush=True)
...
def print_debug_info(file: TextIO = sys.stdout) -> None:
    ...
    print("Printing MONAI_ config...", file=file, flush=True)
    print("================================", file=file, flush=True)
    print_config(file)
